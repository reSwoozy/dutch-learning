const App = {
  ...
  renderProgress(el) {
    ...
    const dueCount = SRS.getDueCount(this.progress.data);
    const testResults = this.progress.data.testResults || {};
    const serverOk = ProgressStore.isServerReachable();
    const hasBuffer = !!ProgressStore.readBuffer();
    ...
    el.innerHTML = `
      ${this.pageHero(
        'Прогресс',
        'Статистика уроков, грамматики, карточек и тестов. Экспорт и локальное хранилище.',
        [
          { text: `${stats.streak} дн. подряд`, muted: true },
      ...
      <div class="card" style="margin-bottom:1.5rem">
        ...
        <p style="color:var(--text-muted);font-size:.9rem">Прогресс хранится в файле <code>dutch-progress.json</code> в корне проекта. ${serverOk
          ? 'Изменения автоматически пишутся на диск через локальный сервер.'
          : 'Сервер сейчас недоступен — изменения временно сохраняются в браузере и будут отправлены на диск при следующем запуске лаунчера.'}</p>
        ${hasBuffer ? '<p style="color:var(--red);font-size:.85rem;margin-top:.5rem">Есть несохранённые изменения в браузерном резерве.</p>' : ''}
...
const ProgressStore = {
  BUFFER_KEY: 'dutch-progress-buffer',
  LEGACY_KEY: 'dutch-progress',
  FILE_URL: '../dutch-progress.json',
  ...
  async readFromServer() {
    try {
      const res = await fetch(this.FILE_URL, { cache: 'no-store' });
      if (!res.ok) {
  ...
  async writeToServer(data) {
    try {
      const res = await fetch(this.FILE_URL, {
        method: 'PUT',
  ...
  readBuffer() {
    try {
      const raw = localStorage.getItem(this.BUFFER_KEY);
      if (raw) return JSON.parse(raw);
      const legacyRaw = localStorage.getItem(this.LEGACY_KEY);
      if (legacyRaw) {
        localStorage.setItem(this.BUFFER_KEY, legacyRaw);
  ...
  writeBuffer(data) {
    try {
      localStorage.setItem(this.BUFFER_KEY, JSON.stringify(data));
    } catch {}
  },
...
const Progress = {
  ...
  async load() {
    const buffer = ProgressStore.readBuffer();
    const server = await ProgressStore.readFromServer();

    let data;
    if (buffer) {
      data = buffer;
      if (server.ok) {
        const flushed = await ProgressStore.writeToServer(buffer);
        if (flushed) ProgressStore.clearBuffer();
      }
    } else if (server.ok) {
      data = server.data || this.getDefault();
    } else {
  ...
  async save() {
    ...
    const ok = await ProgressStore.writeToServer(this.data);
    if (ok) {
      ProgressStore.clearBuffer();
      this._offlineWarned = false;
    } else {
      ProgressStore.writeBuffer(this.data);
      if (!this._offlineWarned && typeof App !== 'undefined' && App && typeof App.showToast === 'function') {
        App.showToast('Сервер недоступен — изменения в резерве. Перезапустите лаунчер.', 'error');
  ...
  unmarkLessonCompleted(lessonId) {
    this.data.lessonsCompleted = this.data.lessonsCompleted.filter((id) => id !== lessonId);
    this.save();
  },
  ...
  getStats() {
    return {
      ...
      correctRate: this.data.totalAnswered > 0
